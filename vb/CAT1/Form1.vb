...
Public Class frmSphecyBox
    Private Sub btnCalculate_Click(sender As Object, e As EventArgs)
        ...
        TotalVolume = (boxlength * boxwidth * boxheight) + (pi * radius ^ 2 * Cylinderheight) + (2 / 3 * pi * radius ^ 3)
        TotalSurfaceArea = 2 * ((boxlength * boxwidth) + (boxlength * boxheight)) + (pi * radius * Cylinderheight) + (2 * pi * radius ^ 2)

        MsgBox("Box length= " & boxlength & vbNewLine & "Box width = " &
               boxwidth & vbNewLine & "Box height =" &
               boxheight & vbNewLine & "Cylinder radius = " &
               Cylinderradius & vbNewLine & "The volume of the model is : " &
